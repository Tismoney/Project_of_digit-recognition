...
class NerNet(QObject):
	...
	def get_weight(self, path = "Weight", conv = False):
		...
		for i in range(num_files):
		    ...
		    if ( (conv == True) and (i == 0) ):
				weight = weight.reshape(32, 1, 5, 5)
		    weights.append(weight)
		return weights

	def make_and_fit(self, path):
		...
		if (path == "Weight/Dense3"):
			self.put_weights(all_weights, path = "Weight/Dense3")
		elif (path == "Weight/Dense1"):
			self.put_weights(all_weights, path = "Weight/Dense1")
		elif (path == "Weight/Conv"):
			self.put_weights(all_weights, path = "Weight/Conv", conv = True)


	def make_and_get(self, path):
		input_X = T.tensor4('Input')
		target_y = T.vector('Target', dtype='int32')
